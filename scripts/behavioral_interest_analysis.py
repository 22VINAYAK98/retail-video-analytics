...
def resolve_roi_roles(rois):
    """
    Resolve semantic roles from the ACTUAL ROI names in
    configs/entrance.yaml.
        ...
        Anything else
            -> not assigned to a semantic state here.
              classify_region() will return NA for it.
    """
    ...
    for name in rois:
        ...
        if any(
            keyword in lname
            for keyword in ENTERING_KEYWORDS
        ):
    ...
    return entering, inside, outside
...
def classify_region(point, rois, entering_names, inside_names, outside_names):
    """
    IMPORTANT:
    ...
    outside_roi = point_in_any_roi(
        point,
        rois,
        outside_names
    )
    if outside_roi is not None:
        return "OUT", outside_roi

    return "NA", None


# ============================================================
# Local speed trend (unrelated to the entry bug -- unchanged)
# ============================================================
...
def draw_rois(frame, rois, entering_names, inside_names, outside_names):
    """
...
def write_track_csv(states, path):
    ...
    with open(path, "w", newline="") as f:
        ...
        for tid in sorted(states):
            ...
            frames = [m["frame"] for m in s.metrics]
            ...
            w.writerow({
                ...
                "number_of_roi_samples": len(s.region_history),
            })
...
def main():
    ...
    entering_names, inside_names, outside_names = resolve_roi_roles(
        rois
    )
        ...
        if (
            ...
            for box, tid in zip(
                ...
                if tid not in states:
                    states[tid] = PersonState(tid, trail_length=TRAIL_LENGTH)
